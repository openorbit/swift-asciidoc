...
public struct ScoreExtension: AsciiDocExtension {
    public let name = "score"
    ...
    private func transform(block: AdocBlock) -> AdocBlock {
        ...
        case .paragraph(let paragraph):
            if let converted = convertScoreTextBlock(
                text: paragraph.text,
    ...
    private func convertScoreTextBlock(
        text: AdocText,
        title: AdocText?,
        reftext: AdocText?,
        meta: AdocBlockMeta,
        ...
        guard meta.attributes["style"]?.lowercased() == "score" else {
            return nil
        }
        ...
        normalizedMeta.attributes.removeValue(forKey: "3")

        if normalizedMeta.options.contains("tabs") || positionalFlags.contains("tabs") {
            normalizedMeta.attributes["tabs"] = "true"
        }
        if normalizedMeta.options.contains("audio") || positionalFlags.contains("audio") {
            normalizedMeta.attributes["audio"] = "true"
        }
